...
class DNN(nn.Module):
    def __init__(self, inputs_dim, hidden_units, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
        ...
        for name, tensor in self.linears.named_parameters():
            if 'weight' in name:
                nn.init.normal_(tensor, mean=1.0/tensor.shape[0], std=init_std)
        self.to(device)
...
class NeuFMRecommender(nn.Module):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument("--model_path",type=str, default='./models/NeuFM/')
        ...
        parser.add_argument("--dnn_hidden_units",type=str, default="64,64")
    ...
    def __init__(self,opt):
        super(NeuFMRecommender, self).__init__()
        self.opt = opt
        self.model_names = ["user","item"]
        self.gpu = opt.gpu
        self.feat_sizes = opt.feat_sizes
        self.embedding_size = opt.embedding_size
        self.dnn_hidden_units = [int(i) for i in self.opt.dnn_hidden_units.split(',')]
    ...
    def forward(self, user, item_i, item_j):
        ...
        linear_i = self.linear(linear_i)
        linear_j = self.linear(linear_j)

        user = self.embed_user(user)

        item_i = self.embed_item(item_i)
        prediction_i = (user * item_i)

        dnn_prediction_i = self.dnn(prediction_i)
        dnn_prediction_i = self.dnn_linear(dnn_prediction_i)


        item_j = self.embed_item(item_j)
        prediction_j = (user * item_j)
        dnn_prediction_j = self.dnn(prediction_j)
        dnn_prediction_j = self.dnn_linear(dnn_prediction_j)


        return dnn_prediction_i+0.3*linear_i,dnn_prediction_j+0.3*linear_j
    ...
    def preprocess(self, train_data,test_data, user_id, item_id):
        ...
        self.dnn = DNN(self.embedding_size, self.dnn_hidden_units, activation="leakyrelu",
                       l2_reg=0, dropout_rate=0, use_bn=False,
                       init_std=0.01, device="cpu")


        self.dnn_linear = nn.Linear(self.dnn_hidden_units[-1], 1, bias=False).to("cpu")
        nn.init.normal_(self.dnn_linear.weight, std=0.01)
        self.dnn_linear.weight.data.add_(1)
    ...
    def ouput_topN_recommender(self, opt, recommend_list):
        ...
        path = opt.dataroot + "/NFM_result.txt"
